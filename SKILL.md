...
- **Pop→Club rework (M4, P66)**: `club_rework.py --pop pop.wav --drums-donor club.wav --demix-dir <кэш> [--target-bpm 124] --out club_edit.wav` — из попсы КЛУБНАЯ версия: DJ-структура по A1F bar_labels (луп-интро 16 бар → тело → луп-аутро), drums попсы −12dB + грув из drums-стема клубного донора (тайл по всей длине), общий стретч к клубному темпу (гейт 0.85–1.25 — рассчитан на типовой кейс «попса 104 → клуб 124»; --force за пределами). Требует A1F+стемы обоих треков (batch_a1f). Opt-in, smart_mixer не тронут.
