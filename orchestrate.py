...
def build_plan(name: str, path: str = "b", config: str = "", style: str = "",
               ...
               tracklist: str = "", year_lo: int = 0, year_hi: int = 0) -> list[tuple]:
    ...
    if tracklist:
        ...
        genre = style or tag
        if not genre:
            raise ValueError("source discogs требует --style/--tag (стиль Discogs-таксономии)")
        import datetime as _dt
        y_hi = year_hi or _dt.date.today().year
        y_lo = year_lo or (y_hi - 1)
        plan.append(("discogs", ["python3", "discogs_source.py", "--style", genre,
                                 "--year-min", str(y_lo), "--year-max", str(y_hi),
                                 "--target", str(target), "--out", cand]))
        urls = f"{name}_urls.txt"
    elif source == "auto":
...
def _main():
    ...
    ap.add_argument("--source", choices=["youtube", "beatport", "discogs", "auto"], default="auto",
    ...
    ap.add_argument("--year-min", type=int, default=0, help="год-констрейнт брифа (Фаза 1б)")
    ...
    plan = build_plan(args.name, args.path, args.config, args.style, args.artists,
                      ...
                      tracklist=args.tracklist, year_lo=args.year_min, year_hi=args.year_max)
